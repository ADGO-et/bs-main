...
export default function PaymentStat({ majorText, innerText }: SuccessProps) {
  ...
  const overallStatus: string | undefined = verifyResult?.status || paymentData?.status

  const terminalSuccess = ["success", "successful"].includes((paymentData?.status || "").toLowerCase())
  const terminalFailed = ["failed", "declined"].includes((paymentData?.status || "").toLowerCase())
  ...
  const heading = useMemo(() => {
    ...
    return `Payment Status: ${paymentData?.status || overallStatus || "Unknown"}`
  }, [majorText, txRef, verifying, verifyError, terminalSuccess, terminalFailed, paymentData?.status, overallStatus])
  ...
  const subText = useMemo(() => {
    ...
    if (terminalFailed) return paymentData?.error ? `Reason: ${paymentData.error}` : "The payment did not complete."
    return "Current status returned from gateway."
  }, [innerText, txRef, verifying, verifyError, terminalSuccess, terminalFailed, paymentData?.error, error])
      ...
      <Card className="w-full max-w-lg mx-auto shadow-xl border-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
        <CardContent className="p-8 space-y-6 text-center">
            ...
            <div className="relative">
              ...
              {terminalFailed ? (
                ...
                <Checkmark
                  size={80}
                  strokeWidth={3}
                  color={terminalSuccess ? "rgb(16 185 129)" : "rgb(59 130 246)"}
          ...
          >
            ...
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">{subText}</p>
          </motion.div>
          ...
          {txRef && (
            ...
            >
              <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">Transaction ID: {txRef}</span>
            </motion.div>
          )}
          ...
          {verifying && (
            ...
            >
              ...
              <span className="text-sm font-medium">Contacting payment gateway...</span>
            </motion.div>
          )}
          ...
          {paymentData && (
            ...
            >
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Payment Details</h3>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-700">
                    <span className="font-medium text-slate-600 dark:text-slate-400">Amount</span>
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <span className="text-slate-900 dark:text-slate-100">
                      {[paymentData.first_name, paymentData.last_name].filter(Boolean).join(" ") || "—"}
                    </span>
                  </div>
                  {paymentData.email && (
                    <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-700">
                  ...
                  {paymentData.error && (
                    <div className="flex justify-between items-start py-2 border-b border-slate-200 dark:border-slate-700">
                      <span className="font-medium text-slate-600 dark:text-slate-400">Error</span>
                      <span className="text-red-600 dark:text-red-400 text-right">{paymentData.error}</span>
                    </div>
                  )}
                  {paymentData.created_at && (
                    <div className="flex justify-between items-center py-2">
                      <span className="font-medium text-slate-600 dark:text-slate-400">Payment Date</span>
          ...
          >
            <Button
              onClick={() => router.push("/")}
              className="w-full bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-slate-200 dark:text-slate-900 text-white font-medium py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl"
            >
              Back to Home
            </Button>
          </motion.div>
